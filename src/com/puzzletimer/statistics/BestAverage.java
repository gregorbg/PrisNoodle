    ...
    public void setSolutions(Solution[] solutions) {
        ...
        for (int i = 0; i < times.length - this.minimumWindowSize + 1; i++) {
            ...
            for (int j = 0; j < this.minimumWindowSize; j++) {
                ...
                if (!solutions[i].getSessionId().equals(solutions[i + j].getSessionId())) {
                    sum = Long.MAX_VALUE;
                    break;
                }

                if (times[i + j] > worst) worst = times[i + j];

                if (times[i + j] < best) best = times[i + j];

                sum += times[i + j];
            }

            if (sum == Long.MAX_VALUE) continue;
