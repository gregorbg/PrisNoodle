...
public class SolutionDAO {
    ...
    public Solution[] getAll(Category category) {
        ...
        ScrambleParser scramblerParser = this.scrambleParserProvider.get(scrambler.getPuzzle().getPuzzleId());

        ArrayList<Solution> solutions = new ArrayList<>();

        try {
            PreparedStatement statement = this.connection.prepareStatement(
                    "SELECT SOLUTION_ID, CATEGORY_ID, SESSION_ID, SCRAMBLER_ID, SEQUENCE, START, END, PENALTY " +
                            "FROM SOLUTION " +
                            "WHERE CATEGORY_ID = ? " +
                            "ORDER BY START DESC");
            ...
            while (resultSet.next()) {
                ...
                UUID sessionId = UUID.fromString(resultSet.getString(3));
                String scramblerId = resultSet.getString(4);
                String sequence = resultSet.getString(5);
                Date start = resultSet.getTimestamp(6);
                Date end = resultSet.getTimestamp(7);
                String penalty = resultSet.getString(8);

                Scramble scramble = new Scramble(scramblerId, scramblerParser.parse(sequence));
                Solution solution = new Solution(solutionId, categoryId, sessionId, scramble, new Timing(start, end), penalty);
    ...
    public void insert(Solution[] solutions) {
        try {
            ...
            for (Solution solution : solutions) {
                ...
                statement.setString(3, solution.getSessionId().toString());
                statement.setString(4, solution.getScramble().getScramblerId());
                statement.setString(5, solution.getScramble().getRawSequence());
                statement.setTimestamp(6, new Timestamp(solution.getTiming().getStart().getTime()));
                statement.setTimestamp(7, new Timestamp(solution.getTiming().getEnd().getTime()));
                statement.setString(8, solution.getPenalty());
