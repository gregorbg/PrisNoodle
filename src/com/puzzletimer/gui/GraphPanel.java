...
public class GraphPanel extends JPanel {
    ...
    public void setSolutions(Solution[] solutions) {
        ...
        if (this.solutionTimes.size() == 0) {
            ...
            // mean
            long mean = 0;
            for (Long solutionTime : this.solutionTimes) mean += solutionTime;
            mean /= this.solutionTimes.size();

            // standard deviation
            long variance = 0;
            for (Long solutionTime : this.solutionTimes) variance += Math.pow(solutionTime - mean, 2d);
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < nVerticalTicks; i++) {
            ...
            String label = SolutionUtils.format(value, false);
        ...
        for (int i = 0; i < nHorizontalTicks; i++) {
            ...
            String label;
            label = this.startIntervalEnd - this.startIntervalStart < 24 * 60 * 60 * 1000
        ...
        ArrayList<ArrayList<Long>> bins = new ArrayList<>(nBins);
        for (int i = 0; i < nBins; i++) bins.add(new ArrayList<>());
        ...
        for (int i = 0; i < nBins; i++)
            if (bins.get(i).size() > 0) {
                long mean = 0;
                for (long time : bins.get(i)) mean += time;
