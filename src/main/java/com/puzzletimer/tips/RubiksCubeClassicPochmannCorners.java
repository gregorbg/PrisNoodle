...
public class RubiksCubeClassicPochmannCorners implements Tip {
    ...
    public String getTip(Scramble scramble) {
        GPuzzle bldPuzzle = WCAPuzzle.THREE.gPuzzle("2012BILL01");
        BldAnalysis analysis = bldPuzzle.getAnalysis(scramble.parseFor(WCAPuzzle.THREE));

        String[] speffzScheme = analysis.getLetteringScheme(new PieceType("CORNER", 8, 3));
        List<List<StickerTarget>> foo = analysis.compileSolutionCycles(new PieceType("CORNER", 8, 3));

        List<String> stickerSequence = new ArrayList<>();
        for (List<StickerTarget> cycle : foo) {
            List<String> speffzCycle = new ArrayList<>();

            for (StickerTarget target : cycle) {
                speffzCycle.add(speffzScheme[target.getTarget()]);
            }

            stickerSequence.addAll(speffzCycle);
        }
